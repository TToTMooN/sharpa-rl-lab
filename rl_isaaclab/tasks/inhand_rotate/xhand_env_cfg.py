...
class XhandEnvCfg(SharpaWaveEnvCfg):
    ...
    object_cfg: RigidObjectCfg = RigidObjectCfg(
        ...
        # Cylinder spawn matches grasp gen (EXP-028).
        init_state=RigidObjectCfg.InitialStateCfg(pos=(-0.085, 0.0, 0.605), rot=(1.0, 0.0, 0.0, 0.0)),
    )
    ...
    # Reduced penalty weights for xhand. SharpaWave defaults punish contact
    # work heavily (-0.5) which is fine for its 22-DOF wrap, but xhand's
    # forced side-pinch generates large initial contact forces that dominate
    # the reward and prevent learning. Halve work + torque penalties.
    work_penalty_scale = -0.1
    torque_penalty_scale = -0.02

    # --- override grasp cache path for xhand ---
    # cache/xhand_grasp_linspace_1.0-1.0-1.npy generated by EXP-028 incremental
    # save mode (rl_isaaclab/tasks/inhand_rotate/sharpa_wave_grasp_env.py
    # _get_rewards saves cond-true states without requiring full episode survival).
