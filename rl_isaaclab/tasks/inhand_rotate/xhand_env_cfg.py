...
class XhandEnvCfg(SharpaWaveEnvCfg):
    ...
    # Cut penalties + boost survival reward 100x to give PPO a clear positive
    # signal for "keeping the sphere in the hand" before it can learn to rotate.
    object_linvel_penalty_scale = -0.05
    pos_diff_penalty_scale = -0.05
    torque_penalty_scale = -0.01
    work_penalty_scale = -0.05
    # Was 0.003, now 0.3 — at default position 1/0.001 ≈ 1000, so reward
    # ≈ 300 per step. Clearly positive when sphere is held.
    object_pos_reward_scale = 0.3

    # disable_height_reset experiment (EXP-034) made it WORSE (-68k mean) because
    # the agent has no way to recover from a dropped sphere — episodes just
    # accumulate penalty for the full length. Reverted.
    # disable_height_reset = True
